"""
DM Topics table - derived index for topic-based search in DMs.

This table is DERIVED from dm_memories.topic - it's automatically
maintained as a topic index for efficient semantic topic search.

Topics are created/updated when DM memories are added, and provide:
1. Semantic topic search (by name/summary)
2. Memory counts per topic
3. Topic evolution tracking per user
"""
...
class DMTopicsTable:
    """CRUD for dm_topics table (derived topic index for DMs)."""
    ...
    def _init_table(self):
        ...
        schema = pa.schema([
            ...
            pa.field("name", pa.string()),  # "defi", "trading", "nft"
        ...
        self._create_vector_index(table, "dm_topics")
    ...
    def upsert_topic(
        ...
        """
        Create or update a DM topic entry.

        Args:
            user_id: User identifier
            name: Topic name (e.g., "defi", "trading")
        ...
        Returns:
            topic_id
        """
        topic_id = f"dm_{user_id}:{name}"

        # Check if topic exists
        existing = self.table.search().where(
            f"user_id = '{user_id}' AND name = '{name}'"
        ).to_list()
        ...
        if existing:
            ...
            if not summary:
                summary = f"Discussions about {name} with {user_id}"
    ...
    def search_semantic(self, user_id: str, query_vector, limit: int = 5) -> List[dict]:
        """
        Search DM topics by semantic similarity.
    ...
    def get_topics_for_user(self, user_id: str) -> List[dict]:
        """Get all topics for a user, sorted by memory count."""
        if self.table.count_rows() == 0:
            return []
    ...
    def get_topic_memory_count(self, user_id: str, topic_name: str) -> int:
        """Get memory count for a specific topic."""
        result = self.table.search().where(
            f"user_id = '{user_id}' AND name = '{topic_name}'"
        ).to_list()
    ...
    def batch_update_memory_counts(self, user_id: str, topic_updates: dict[str, int]):
        """
        ...
        for topic_name, count_delta in topic_updates.items():
            if count_delta != 0:
                topic_id = f"dm_{user_id}:{topic_name}"
                existing = self.table.search().where(
                    f"topic_id = '{topic_id}'"
                ).to_list()
                ...
                if existing:
                    ...
                    if new_count > 0:
                        self.upsert_topic(user_id, topic_name, memory_count_delta=new_count)
