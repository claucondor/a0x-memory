"""
InteractionMemories table operations.
...
from models.group_memory import InteractionMemory, MemoryLevel, MemoryType, PrivacyScope
from database.base import LanceDBConnection
from utils.embedding import EmbeddingModel
import config


class InteractionMemoriesTable:
    """CRUD for interaction_memories table."""
    ...
    def _init_table(self):
        ...
        self._create_scalar_index(table, "speaker_id")
        self._create_scalar_index(table, "listener_id")
        self._create_vector_index(table, "interaction_memories")
    ...
    def add(self, memory: InteractionMemory, dedup_threshold: float = 0.85) -> InteractionMemory:
        """Add a single interaction memory with deduplication."""
            ...
            try:
                ...
                if similar:
                    ...
                    if similarity >= dedup_threshold:
                        print(f"[dedup] Skip interaction memory (sim={similarity:.2f}): {memory.content[:40]}...")
                        return memory
    ...
    def _find_similar_existing(self, vector: List[float], group_id: str, threshold: float = 0.85) -> dict:
    ...
    def add_batch(self, memories: List[InteractionMemory]) -> List[InteractionMemory]:
        """Add multiple interaction memories with deduplication."""
        if not memories:
            return []
        ...
        print(f"[{self.agent_id}] Added {new_count} new interaction memories, merged {merged_count}")
        return memories

    def search_semantic(self, group_id: str, speaker_id: str = None, listener_id: str = None, query_vector=None, limit: int = 10):
        ...
        if speaker_id:
            conditions.append(f"speaker_id = '{speaker_id}'")
        if listener_id:
            conditions.append(f"listener_id = '{listener_id}'")
    ...
    def search_keyword(self, group_id: str, keywords: List[str], top_k: int = 5) -> List[tuple]:
        ...
        try:
            ...
            print(f"Error during interaction keyword search: {e}")
            return []
